...
public class SqlEditorWindow {
   ...
   def SqlEditorWindow() {
      ...
      JFrame frame = swing.frame(title: TITLE, id: 'mainFrame', defaultCloseOperation: WindowConstants.EXIT_ON_CLOSE) {
         ...
         actionOptionsSetTitle = swing.action(name: 'Set window title', accelerator: 'ctrl W') {
            def pane = swing.optionPane()
            def title = pane.showInputDialog(null,
...
//               menuItem 'Preferences'
   ...
   private def setWindowTitle() {
      def title = ''
      if (windowTitle && !windowTitle.isEmpty())
         title += windowTitle + ' - '

      title += TITLE

      if (activeFile && !activeFile.isEmpty())
         title += ' - ' + activeFile
